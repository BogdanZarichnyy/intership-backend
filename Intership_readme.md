...
тут мається на увазі що якщо користувач буде брати учать у декількох вікторинах одразу, тобто я так зрозумів що поле quiz_id, це не просто ідентифікатор конкретного тесту - це масив тестів в одній вікторині, або id прив'язка до окремої таблиці із тестів. Тобто оцінка для користувача по питаннях, і цей останній час активності буде разуватися на основі змін в таблиці з тестами?- я так розумію що це буде ще одна таблиця, але вона буде суто пов'язана із таблицею quiz_results???
хоча потрібно запитати у ментора на рахунок такої логіки, бо з ТЗ це неясно